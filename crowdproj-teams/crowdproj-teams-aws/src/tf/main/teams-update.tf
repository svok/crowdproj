...
resource "aws_lambda_permission" "teams_update" {
  ...
  function_name = aws_lambda_function.teams_update_post.arn
  principal = "apigateway.amazonaws.com"
  source_arn = "arn:aws:execute-api:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.back_app.id}/*/POST/teams/update"
}
...
resource "aws_lambda_function" "teams_update_post" {
  function_name = "v001-teams-update-post"
  role = aws_iam_role.teams_update_post.arn
  ...
  handler = var.handlers["teams-update"]
  runtime = "java8"
  timeout = 300
  memory_size = 1024
  environment {
...
resource "aws_iam_role" "teams_update_post" {
  name = "v001-teams-update-post"
  assume_role_policy = data.aws_iam_policy_document.merged_0_assume.json
}

resource "aws_iam_role_policy" "crowdproj_teams_update_post" {
  role = aws_iam_role.teams_update_post.name
  policy = data.aws_iam_policy_document.crowdproj_teams_table.json
}

resource "aws_cloudwatch_log_group" "teams_update_post" {
  name              = "/aws/lambda/${aws_lambda_function.teams_update_post.function_name}"
  retention_in_days = 14
}

resource "aws_iam_role_policy_attachment" "teams_update_post" {
  role       = aws_iam_role.teams_update_post.name
