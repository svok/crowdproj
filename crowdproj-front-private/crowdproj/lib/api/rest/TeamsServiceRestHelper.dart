...
class TeamsServiceRestHelper {
  ...
  static local.Team fromTeam(remote.Team team) => local.Team(
    id: team.id,
    name: team.name,
    summary: team.summary,
    description: team.description,
    owner: local.Profile.fromExchange(team.owner),
  ...
  static remote.TeamBuilder toTeamBuilder(local.Team local) => remote.TeamBuilder()
    ..id = local.id
    ..name = local.name
    ..summary = local.summary
    ..description = local.description
    ..owner = local.owner.toExchangeBuilder()
    ..visibility = toVisibility(local.visibility)
    ..joinability = toJoinability(local.joinability)
    ..status = toStatus(local.status);

  static local.TeamVisibility fromVisibility(remote.TeamVisibility remote) => local.TeamVisibility.values
      .firstWhere((e) => e.toString().replaceAll("_", "").toLowerCase() == remote.name.toLowerCase(),
      orElse: () => null); //return null if not found

  static remote.TeamVisibility toVisibility(local.TeamVisibility local) => remote.TeamVisibility.values
      .firstWhere((e) => e.toString().replaceAll("_", "").toLowerCase() == local.toString().toLowerCase(),
      orElse: () => null); //return null if not found

  static local.TeamJoinability fromJoinability(remote.TeamJoinability remote) => local.TeamJoinability.values
      .firstWhere((e) => e.toString().replaceAll("_", "").toLowerCase() == remote.name.toLowerCase(),
      orElse: () => null); //return null if not found

  static remote.TeamJoinability toJoinability(local.TeamJoinability local) => remote.TeamJoinability.values
      .firstWhere((e) => e.toString().replaceAll("_", "").toLowerCase() == local.toString().toLowerCase(),
      orElse: () => null); //return null if not found

  static local.TeamStatus fromStatus(remote.TeamStatus remote) => local.TeamStatus.values.firstWhere(
          (e) => e.toString().replaceAll("_", "").toLowerCase() == remote.name.toLowerCase(),
      orElse: () => null); //return null if not found

  static remote.TeamStatus toStatus(local.TeamStatus local) => remote.TeamStatus.values.firstWhere(
          (e) => e.toString().replaceAll("_", "").toLowerCase() == local.toString().toLowerCase(),
      orElse: () => null); //return null if not found
